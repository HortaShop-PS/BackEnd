...
export class ProductService {
  ...
  async createProduct(
    createProductDto: CreateProductDto & { producerId: number },
  ) {
    ...
    if (!producer) {
      throw new NotFoundException(
        `Produtor com userId ${createProductDto.producerId} não encontrado`,
      );
    }
  ...
  async getAllProducts(): Promise<
    (Product & { averageRating?: number; totalReviews?: number })[]
  > {
    try {
      const products = await this.repo.find({
